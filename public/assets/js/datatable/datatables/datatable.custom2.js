(function ($) {
    $(document).ready(function () {
        ...
        // Suppress DataTables "Requested unknown parameter" warning globally
        $.fn.dataTable.ext.errMode = 'none';

        // Helper: init DataTable safely (handles empty tables without warning)
        function safeDataTable(selector, options) {
            var $el = $(selector);
            if (!$el.length) return;
            // Build columnDefs that fills any missing cell with empty string
            var defaults = {
            ...
            $el.DataTable($.extend(true, {}, defaults, options || {}));
        }

        // Basic table example
        safeDataTable("#basic-1");
        safeDataTable("#table-dosen");
        safeDataTable("#table-mahasiswa");
        safeDataTable("#basic-2", {
            paging: true,
            ordering: false,
            info: false,
            autoWidth: false,
        });
        safeDataTable("#basic-6");
        safeDataTable("#basic-9", { stateSave: true });
        safeDataTable("#basic-12", {
            scrollY: "40vh",
            scrollCollapse: true,
            paging: false,
        });
